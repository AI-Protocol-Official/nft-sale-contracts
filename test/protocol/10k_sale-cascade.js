// AI Protocol FixedSupplySale: Cascade Sale Tests
...
// run sale tests
contract("FixedSupplySale: Cascade Sale Tests", function(accounts) {
	...
	before(async function() {
		...
		await ali.transfer(H0, await ali.totalSupply(), {from: a0});
		await ali.approve(sale.address, MAX_UINT256, {from: H0});
	});
	...
	for(let i = 0; i < cascades; i++) {
		describe(`Cascade ${i}/${cascades}`, function() {
			...
			before(async function() {
				...
				const batch_size = 0;
				const ali_source = H0;
				const ali_value = toWei("2000", "ether");
