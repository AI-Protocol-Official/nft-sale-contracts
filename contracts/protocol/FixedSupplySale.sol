...
import "@ai-protocol/intelligent-nft-contracts/contracts/protocol/IntelligentNFTv2.sol";
...
contract FixedSupplySale is AccessControl {
	 ...
	 */
	constructor(address _ali, address _nft, address _personality, address _iNft) AccessControl(msg.sender, 0) {
	...
	function initialize(
		...
		require(
			(_aliSource == address(0) && _aliValue == 0) || (_aliSource != address(0) && _aliValue >= 1e12),
			"invalid ALI source/value"
		);
		...
		emit Initialized(msg.sender, itemPrice, nextId, finalId, saleStart, saleEnd, batchLimit, aliSource, aliValue);
	}
	 ...
	 */
	function buy(uint32 _amount) public payable virtual {
	 ...
	 *      will be created and bound/locked to iNFT), two or more
	 */
	function buyTo(address _to, uint32 _amount) public payable virtual {
		// check the access permission
	 ...
	 *      Accepts ETH as payment and creates iNFT with minted bound NFT and minted linked AI Personality
	 */
	function buySingle() public payable virtual {
	 ...
	 */
	function buySingleTo(address _to) public payable virtual {
		// check the access permission
		require(isFeatureEnabled(FEATURE_PUBLIC_SALE) || isSenderInRole(ROLE_BUYER), "access denied");
